...
> & {
  ...
  i18nkey?: string;
  version: string;
  /** Optional Antdv Next theme overrides */
  antdv?: App.Theme.AntdvThemeOverride;
};
...
const applyPreset = (preset: ThemePreset): void => {
  ...
  const { themeScheme, grayscale, colourWeakness, layout, watermark, antdv, ...rest } = mergedPreset;
  ...
  themeStore.setAntdvThemeOverrides(antdv);
    ...
    >
      <div class="mb-2 flex items-center justify-between">
        <div class="min-w-0 w-full flex flex-1 items-center justify-between gap-2">
          ...
          <ABadge :count="`v${preset.version}`" color="blue" class="flex-shrink-0 opacity-80" />
        </div>
        <AButton
          type="primary"
          size="small"
